...
After a successful connection, GivLocal remembers the last working connection method and reuses it on later connects, so reconnecting after returning to the app is usually instant. If you mark a device as a favourite (available with **GivLocal Pro**), it can connect automatically on launch.
